 ...
 *
 * 流程：
 * 0(待付定金) → 1(已付定金/待商家确认) → 2(已确认/待付尾款)
 * → 3(已付全款/待入住) → 4(已入住) → 5(已完成)
 * → 6(已取消) / 7(已拒绝) / 8(已关闭)
 */
export const ORDER_STATUS = {
  PENDING_DEPOSIT: 0,      // 待付定金
  DEPOSIT_PAID: 1,         // 已付定金，待商家确认
  CONFIRMED: 2,            // 商家已确认，待付尾款
  FULLY_PAID: 3,           // 已付全款，待入住
  CHECKED_IN: 4,           // 已入住
  COMPLETED: 5,            // 已完成
  CANCELLED: 6,            // 已取消
  REJECTED: 7,             // 已拒绝
...
export const ORDER_STATUS_MAP: Record<number, { label: string; color: string; desc: string }> = {
  ...
  2: { label: '待付尾款', color: '#e6a23c', desc: '订单已确认，请支付尾款' },
  3: { label: '待入住', color: '#67c23a', desc: '费用已结清，请按时入住' },
  4: { label: '已入住', color: '#409eff', desc: '祝您玩得开心！' },
