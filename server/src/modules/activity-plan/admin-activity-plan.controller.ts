...
export class AdminActivityPlanController {
  ...
  @Get()
  async list(@AdminCtx() ctx: AdminContext, @Query('page') page?: string, @Query('pageSize') ps?: string) {
    return this.service.findAll(ctx, page ? parseInt(page) : 1, ps ? parseInt(ps) : 10);
  }
